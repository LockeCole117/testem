...
//returns all the files in the "external_files" option.
//I don't think there's any validation we can perform on these files.
//One option would be to check if the host is reachable, but I'd rather let the browser decide that
Config.prototype.getExternalFiles= function(){
    return this.get('external_files') || [] //if no external files are found, return a blank array
}

Config.prototype.getServeFiles = function(callback){
    var serve_files = this.get('serve_files') || this.get('src_files')
    var external_files = this.getExternalFiles()

    if (isa(serve_files, Array)) {
        //since we have confirmed serve_files is an array, concatenate our array of external files to it
        serve_files = serve_files.concat(external_files)
        log.info("new array1: " + serve_files)
        async.reduce(serve_files, [], function(curr, pattern, next){
            glob(pattern, function(err, files){
                if (err){
                    next(null, curr)
                } else{
                    next(null, curr.concat(files))
                }
            })
        }, callback)
    }else{
        fs.readdir(process.cwd(), function(err, files){
            ...
            //now that we have created an array of all the files at the current directory, concatenate our external files
            files = files.concat(external_files)
            log.info("new array2: " + files)
            callback(err, files)
        })
    }
}
